_ghostty() {
  ...
  _handle_config() {
    ...
    case "$prev" in
      ...
      --window-padding-balance) _compreply compgen -W "false true equal" -- "$cur"; _add_spaces ;;
  ...
  _handle_actions() {
    ...
    local show_config="'--default ' '--changes-only ' '--docs ' '--no-pager ' --help"
    local explain_config="--option= --keybind= --help"
    ...
    case "${COMP_WORDS[1]}" in
      ...
      +explain-config)
        case $prev in
          --option) return;;
          --keybind) return;;
          *) _compreply compgen -W "$explain_config" -- "$cur" ;;
        esac
      ;;
      +validate-config)
        case $prev in
